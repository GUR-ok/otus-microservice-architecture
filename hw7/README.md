...
Сервис предоставляет API для работы с заказами. После создания заказа пользователь должен оплатить заказа.
...
CONFIRMED - заказ подтвержден пользователем и требует оплаты. Списание средств со счета в биллинге произойдет автоматически при наличии средств.
...
- `OrderConfirmed` — событие при подтверждении заказа пользователем
